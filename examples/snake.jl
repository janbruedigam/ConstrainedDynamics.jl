...
setPosition!(origin,links[1],p2 = [0;-2.0;0.5],Δq = QuatRotation(RotX(pi/2)*RotZ(pi/4)))
setVelocity!(links[1],v=randn(3)*2,ω=randn(3)*2)
for j=2:N
    if iseven(j)
        setPosition!(links[j-1],links[j],p1 = vert12, p2 = vert11,Δq = QuatRotation(RotY(-pi/2)))
    else
        setPosition!(links[j-1],links[j],p1 = vert12, p2 = vert11,Δq = QuatRotation(RotY(pi/2)))
    end  
