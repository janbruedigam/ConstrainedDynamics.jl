...
g(J::Axis,link1::Origin,link2::Link) = J.V12*Vmat(getq3(link2))

# function ∂g∂posa(J::Axis{T},link1::Origin,link2::Link) where T
#     if SVector{2,Int64}(link1.id,link2.id) == J.linkids
#         X = @SMatrix zeros(T,2,3)
#
#         No = link2.No
#         R = -J.V12*Vmat(VTmat(Rmat(link2.q[No])))
#
#         return [X R]
#     else
#         return ∂g∂posa(J)
...
function ∂g∂posb(J::Axis{T},link1::Origin,link2::Link) where T
    if link2.id == J.link2id
        X = @SMatrix zeros(T,2,3)
...
# function ∂g∂vela(J::Axis{T},link1::Origin,link2::Link) where T
#     if SVector{2,Int64}(link1.id,link2.id) == J.linkids
#         V = @SMatrix zeros(T,2,3)
#
#         No = link2.No
#         Ω = -link2.dt^2/4*J.V12*Vmat(VTmat(Rmat(ωbar(link2))*Rmat(link2.q[No])))
#
#         return [V Ω]
#     else
#         return ∂g∂vela(J)
...
function ∂g∂velb(J::Axis{T},link1::Origin,link2::Link) where T
    if link2.id == J.link2id
        V = @SMatrix zeros(T,2,3)

        No = link2.No
        Ω = link2.dt/2*J.V12*Vmat(Lmat(link2.q[No]))*derivωbar(link2)
