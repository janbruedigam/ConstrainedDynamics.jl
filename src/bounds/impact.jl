...
@inline g(impact::Impact{T}, x::AbstractVector, q::QuatRotation) where T = SVector{1,T}(impact.ainv3 * (x + vrotate(impact.p,q) - impact.offset))

## Derivatives NOT accounting for quaternion specialness
@inline function ∂g∂pos(impact::Impact, x::AbstractVector, q::QuatRotation)
    p = impact.p
    X = impact.ainv3
    Q = impact.ainv3 * (VLmat(q) * Lmat(QuatRotation(p)) * Tmat() + VRᵀmat(q) * Rmat(QuatRotation(p)))
    return X, Q
end
