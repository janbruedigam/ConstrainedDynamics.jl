...
@inline function ∂g∂vela(joint::Rotational2{T}, body1::Body, body2::Body, Δt, No) where T
    if body2.id == joint.cid
        ...
        Ω = joint.V12 * VLᵀmat(joint.qoff) * Rmat(ωbar(body2, Δt)) * Rmat(body2.state.qd[No]) * Rᵀmat(body1.state.qd[No]) * Tmat(T) * derivωbar(body1, Δt)
...
@inline function ∂g∂velb(joint::Rotational2{T}, body1::Body, body2::Body, Δt, No) where T
    if body2.id == joint.cid
        V = @SMatrix zeros(T, 2, 3)
        Ω = joint.V12 * VLᵀmat(joint.qoff) * Lᵀmat(ωbar(body1, Δt)) * Lᵀmat(body1.state.qd[No]) * Lmat(body2.state.qd[No]) * derivωbar(body2, Δt)
...
@inline function ∂g∂velb(joint::Rotational2{T}, body1::Origin, body2::Body, Δt, No) where T
    if body2.id == joint.cid
        V = @SMatrix zeros(T, 2, 3)
        Ω = joint.V12 * VLᵀmat(joint.qoff) * Lmat(body2.state.qd[No]) * derivωbar(body2, Δt)
