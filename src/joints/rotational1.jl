...
@inline g(joint::Rotational1, body1::Body, body2::Body, Δt, No) = joint.V3 * (VRmat(joint.qoff) * Lᵀmat(getq3(body1, Δt)) * getq3(body2, Δt))
...
@inline function ∂g∂posa(joint::Rotational1{T}, body1::Body, body2::Body, No) where T
    if body2.id == joint.cid
        ...
        R = -joint.V3 * VRmat(joint.qoff) * Rmat(body2.q[No]) * RᵀVᵀmat(body1.q[No])
...
@inline function ∂g∂posb(joint::Rotational1{T}, body1::Body, body2::Body, No) where T
    if body2.id == joint.cid
        ...
        R = joint.V3 * VRmat(joint.qoff) * Lᵀmat(body1.q[No]) * LVᵀmat(body2.q[No])
...
@inline function ∂g∂vela(joint::Rotational1{T}, body1::Body, body2::Body, Δt, No) where T
    if body2.id == joint.cid
        ...
        Ω = joint.V3 * VRmat(joint.qoff) * Rmat(ωbar(body2, Δt)) * Rmat(body2.q[No]) * Rᵀmat(body1.q[No]) * Tmat(T) * derivωbar(body1, Δt)
...
@inline function ∂g∂velb(joint::Rotational1{T}, body1::Body, body2::Body, Δt, No) where T
    if body2.id == joint.cid
        ...
        Ω = joint.V3 * VRmat(joint.qoff) * Lᵀmat(ωbar(body1, Δt)) * Lᵀmat(body1.q[No]) * Lmat(body2.q[No]) * derivωbar(body2, Δt)
...
@inline g(joint::Rotational1, body1::Origin, body2::Body, Δt, No) = joint.V3 * VRmat(joint.qoff) * getq3(body2, Δt)
...
@inline function ∂g∂posb(joint::Rotational1{T}, body1::Origin, body2::Body, No) where T
    if body2.id == joint.cid
        ...
        R = joint.V3 * VRmat(joint.qoff) * LVᵀmat(body2.q[No])
...
@inline function ∂g∂velb(joint::Rotational1{T}, body1::Origin, body2::Body, Δt, No) where T
    if body2.id == joint.cid
        ...
        Ω = joint.V3 * VRmat(joint.qoff) * Lmat(body2.q[No]) * derivωbar(body2, Δt)
