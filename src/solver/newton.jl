...
function lineSearch!(mechanism::Mechanism,meritf0;iter=10, warning::Bool=false)
    ...
    for n=Base.OneTo(iter)
        for body in bodies
            lineStep!(body,getentry(ldu,body.id),scale,α)# x1 = x0 - 1/(2^e)*d
        end
        for eqc in eqcs
            lineStep!(eqc,getentry(ldu,eqc.id),scale,α)# x1 = x0 - 1/(2^e)*d
        end
        for ineqc in ineqcs
            lineStep!(ineqc,getineqentry(ldu,ineqc.id),scale,α)# x1 = x0 - 1/(2^e)*d
        end
